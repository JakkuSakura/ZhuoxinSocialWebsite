...
class controller
{
    ...
    public function ajax_crop()
    {
        ...
        if (!file_exists($src_path)) {
            $reponse->success = false;
            $reponse->description = '未找到图片文件' . $src_path;
        } else {
            include('models' . DIRECTORY_SEPARATOR . 'gd.php');
            $gd = new gd();
            $gd->open($src_path);
            if ($gd->is_image()) {
                ...
                $path = "upload/". date("Y-m-d", time()) . "/" . $avatar_name;


                $gd->save_to(ROOT . $path);

                require_once ROOT . "User.php";
                if (User::isLoggedIn())
                    Database::update('user', 'img_path', $path, 'id=' . User::getLoginUser()->id);
